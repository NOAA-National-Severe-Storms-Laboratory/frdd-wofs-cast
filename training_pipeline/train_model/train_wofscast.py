...
# 5. Need to add the forcing variables (radiation) to the dataset, built my own function. 
# 6. Xarray 2024.1.1 raised TracerArrayConversionError, downgraded to 2023.7.0, the version 
#    used in colab in the demo notebook.

""" usage: stdbuf -oL python -u train_wofscast.py > & log_trainer_models & """
...
trainer = WoFSCastTrainer(mesh_size=5, 
                 ...
                 n_epochs_phase2 = 50,
                 n_epochs_phase3 = 0,
                 total_timesteps = 2, # 2+ hours of total rollout for training. 
                 batch_size=64,
                 generator_chunk_size=512,          
                 checkpoint=True,
                 norm_stats_path = '/work/mflora/wofs-cast-data/normalization_stats',
                 out_path = '/work/mflora/wofs-cast-data/model/wofscast_baseline_full.npz',
...
####data_paths = data_paths[:189]

rs = np.random.RandomState(123)

data_paths = rs.choice(data_paths, size=4096, replace=False)

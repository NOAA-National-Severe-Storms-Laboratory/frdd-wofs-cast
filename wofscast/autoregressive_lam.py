...
class Predictor(predictor_base.Predictor):
  ...
  def __call__(self,
    ...
    boundary_conditions = targets_template.copy()  
    ...
    # Turn the boundary conditions to flattend data. 
    flat_bcs, bc_treedef = _get_flat_arrays_and_single_timestep_treedef(boundary_conditions)
 
    scan_variables = (flat_forcings, flat_bcs)
    
    # Add the boundary conditions for the LAM-based model 
    # Assuming spatial dimensions are named 'lat' and 'lon'
    # Adjust dimension names as necessary for your datasets
    ...
    def one_step_prediction(inputs, scan_variables):
      ...
      boundary_conditions =  _unflatten_and_expand_time(flat_bcs, bc_treedef,
                                            target_template.coords['time'])
  ...
  def loss(self,
    ...
    def one_step_loss(inputs, scan_variables):
      ...
      # Update the boundary conditions. 
      predictions = update_boundary_conditions(predictions, target) 

...
class GenCast(predictor_base.Predictor):
    ...
    * Mesh2Grid graph: Graph that contains all nodes. This graph is strictly
    ...
    def __init__(self, model_config: ModelConfig, task_config: TaskConfig):
        ...
        self._meshes = square_mesh.get_hierarchy_of_triangular_meshes(
    ...
    def __call__(
        ...
        **kwargs, 
        ...
        sigma = jnp.bfloat16(sigma) 
        
        c_skip, c_out, c_in = self.get_denoiser_weights(sigma)
        ...
        grid_node_features = self._inputs_to_grid_node_features(
            inputs, c_in*targets_template, forcings
        )
        ...
        output = self._grid_node_outputs_to_prediction(
    ...
    def get_denoiser_weights(self, sigma : float):
        """Get the 'c' weights from Karras et al. sigma_data=1 due to the normalization of the targets.
        Ignoring the c_noise since noise is added in the conditional norm layers. 
        """
        c_skip = 1.0 / (sigma**2 + 1.0) 
        c_out = sigma / (jnp.sqrt(sigma**2 + 1))
        c_in = 1.0 / (jnp.sqrt(sigma**2 + 1))
    ...
    def loss_and_predictions(  # pytype: disable=signature-mismatch  # jax-ndarray
        ...
        **kwargs
        ...
        """Method for computing model loss and its predictions

        When applied, the GenCast model is wrapped in the normalization.InputsAndResiduals
        class, which normalizes the inputs and compute the target residual.
        
        Apply the denoiser from equation D2 from the GenCast paper.

        D(z_t_sigma, inputs, sigma) = c_skip*targets + c_out * GraphCast(c_in*targets, inputs, sigma)
        """
        # TODO:
        # 1. Add different noises along the batch dimension of the targets? 
        # 2. Create the loss weights for the different noise levels. 
        
        # loss_weight = (sigma**2 + 1) / (sigma**2) ; assumes sigma_data = 1 
        ...
        # predictions are the predictor target residuals while targets are 
        # the normalized target residuals. 
        ...
        # Get batch-wise mean, but with the loss_weight. 
    ...
    def _init_mesh_properties(self):
        ...
        mesh_nodes_lon, mesh_nodes_lat = square_mesh.get_mesh_coords(
            self._finest_mesh, self._grid_lat, self._grid_lon
        )
        # Convert to f32 to ensure the lat/lon features aren't in f64.
        self._mesh_nodes_lat = mesh_nodes_lat.astype(np.float32)
        self._mesh_nodes_lon = mesh_nodes_lon.astype(np.float32)
    ...
    def _init_mesh_graph(self) -> typed_graph.TypedGraph:
        ...
        adjacency_matrix = graph_transformer.create_adjacency_matrix(
            senders, receivers, self._num_mesh_nodes
        )
        khop_adj_mat = graph_transformer.compute_k_hop_adjacency_matrix(
            adjacency_matrix, int(self.k_hop)
        )
    ...
    def _init_mesh2grid_graph(self) -> typed_graph.TypedGraph:
        ...
        # mesh triangles.
        (grid_indices, mesh_indices) = square_mesh.in_mesh_triangle_indices(
            grid_size=len(self._grid_lat), mesh=self._finest_mesh
        )
    ...
    def _inputs_to_grid_node_features(
        ...
        stacked_targets = model_utils.dataset_to_stacked(targets)

        stacked_inputs = xarray.concat(
            [stacked_inputs, stacked_forcings, stacked_targets], dim="channels"
        )

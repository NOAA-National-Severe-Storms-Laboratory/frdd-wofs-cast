...
def threshold_tuned_loss(target, prediction, 
    ...
    # Loss calculations
    overpredict_loss = xarray.where(overpredict_small_val_mask , (prediction - target) * small_val_penalty, 0)
    underpredict_loss_mid = xarray.where(underpredict_mid_val_mask, (target - prediction) * mid_val_penalty, 0)
    underpredict_loss_large = xarray.where(underpredict_large_val_mask, (target - prediction) * large_val_penalty, 0)

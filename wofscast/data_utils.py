...
def add_derived_vars(data: xarray.Dataset) -> xarray.Dataset:
    ...
    # Add year progress features if missing
    if 'YEAR_PROGRESS' not in data.data_vars:
        year_progress = get_year_progress(seconds_since_epoch)  # Assuming this function exists
        data.update(
            featurize_progress(
            name=YEAR_PROGRESS,
            dims=batch_dim + ("time",),
    ...
    if 'DAY_PROGRESS' not in data.data_vars:
        longitude_coord = data.coords["lon"]
        day_progress = get_day_progress(seconds_since_epoch, data.coords["lon"].data)  # Assuming this function exists
...
def add_tisr_var(data: xarray.Dataset) -> xarray.Dataset:
    ...
    tisr = solar_radiation.get_toa_incident_solar_radiation_for_xarray(data, use_jit=False)

    ...
    * num_message_passing_steps = N, num_processor_repetitions = M, gives
    ...
    def __init__(
        ...
        *,
        ...
        use_transformer: bool = False,  # Added by Monte
        k_hop: int = 4,
    ...
    def _networks_builder(self, graph_template):
        ...
        def build_transformer(name, output_size):
            network = graph_transformer.Transformer(
                num_heads=self._num_attn_heads,
                num_layers=1,
                output_size = output_size, 
                attn_size=output_size // self._num_attn_heads,
                dropout_rate=0.0,
                widening_factor=1,
                mask=self._adj_mat,
                name=f"{name}_attn_trans",
            )
            return jraph.concatenated_args(network)
        ...
        builder_fn = (
            ...
            else build_mlp_with_maybe_layer_norm
        )

        
        # BUILD THE EMBEDDING MLPS FOR THE NODE AND EDGE FEATURES. 
        ...
        if self._embed_edges:
            embed_edge_fn = _build_update_fns_for_edge_types(
                build_mlp_with_maybe_layer_norm,  # Only MLP for embedding.
                graph_template,
                "encoder_edges_",
            ...
            self._processor_networks.append(
                typed_graph_net.InteractionNetwork(
                    update_edge_fn=_build_update_fns_for_edge_types(
                        build_mlp_with_maybe_layer_norm,  # The Edges updated by MLP
                        graph_template,
                    ...
                    update_node_fn=_build_update_fns_for_node_types(
                        ...
                        builder_fn,  # Nodes updated by Transformer or MLP
                        graph_template,
        ...
        output_kwargs = dict(
            embed_edge_fn=(
                _build_update_fns_for_edge_types(
    ...
    def _process(
        self, latent_graph_0: typed_graph.TypedGraph
    ) -> typed_graph.TypedGraph:
    ...
    def _process_step(
        self, processor_network_k, latent_graph_prev_k: typed_graph.TypedGraph
    ) -> typed_graph.TypedGraph:

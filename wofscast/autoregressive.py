...
class Predictor(predictor_base.Predictor):
    ...
    def _update_inputs(self, inputs, next_frame):
        ...
        return (
            xarray.concat([inputs, predicted_or_forced_inputs], dim="time").tail(
                time=num_inputs
            )
    ...
    def loss(
        ...
        **kwargs,
        ...
        flat_forcings, forcings_treedef = _get_flat_arrays_and_single_timestep_treedef(
            forcings
        )
        ...
        (loss, diagnostics) = jax.tree_util.tree_map(
            lambda x: xarray_jax.DataArray(
                x, dims=("time", "batch")
            ).mean(  # pylint: disable=g-long-lambda
                "time", skipna=False
            ),

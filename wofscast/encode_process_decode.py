...
def GraphNetworkGraphUpdate(
    *,
    ...
    use_input_context_state: bool = False,
...
class EncodeProcessDecode(tf.keras.layers.Layer):
  ...
  * produce per node, per edge and/or context outputs.
  ...
  def __init__(
      ...
      encode_edges : bool, 
      encode_nodes :  bool, 
      edge_output_size: Optional[Union[int, Mapping[str, int]]],
      node_output_size: Optional[Union[int, Mapping[str, int]]],
    ...
    def encoder_fn(graph_piece, *, edge_set_name=None, node_set_name=None):
      ...
      ##print(f"{graph_piece=}, {edge_set_name=}, {node_set_name=}")   
    ...
    edge_sets_fn = encoder_fn if encode_edges else None 
    node_sets_fn = encoder_fn if encode_nodes else None 

    ##print(f"{edge_sets_fn=}, {node_sets_fn=}")
    
    
    self._encoder = tfgnn.keras.layers.MapFeatures(
        edge_sets_fn=edge_sets_fn, node_sets_fn=node_sets_fn)#, context_fn=encoder_fn)
    ...
    def decoder_fn(graph_piece, *, edge_set_name=None, node_set_name=None):
      ...
      node_output_keys = node_output_size.keys()
      if node_set_name not in node_output_keys:
        ###print(f"{node_set_name} has no decoding!")
        return None

      if edge_set_name:
        output_size = edge_output_size
      elif node_set_name:
        output_size = node_output_size[node_set_name]
      else:
  ...
  def call(self, input_graph: tfgnn.GraphTensor) -> tfgnn.GraphTensor:
    ###print("Node sets:", input_graph.node_sets.keys())
